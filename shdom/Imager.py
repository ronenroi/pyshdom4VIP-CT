...
class SensorFPA(object):
    ...
    def assume_QE(self, QE, spectrum):
        ...
        df = pd.DataFrame(
            data={"<wavelength [nm]>": self._SPECTRUM, '<Efficiency>': (QE / 100) * np.ones_like(self._SPECTRUM)},
            index=None)
        self._QE = df
...
class LensSimple(object):
    ...
    def assume_UNITY_TRANSMISSION(self, spectrum):
        ...
        self._SPECTRUM = np.array(
            spectrum)  # need to be more then 4 elements for the InterpolatedUnivariateSpline function in the future.
        df = pd.DataFrame(
            data={"<wavelength [nm]>": self._SPECTRUM, '<Efficiency>': alfa * np.ones_like(self._SPECTRUM)}, index=None)
...
class Imager(object):
    def __init__(self, sensor=None, lens=None, scene_spectrum=[400, 1700], integration_Dlambda=10, temp=20,
                 system_efficiency=1):
        """
        Parameters:
        sensor - sensor class.
        ...
        if (self._scene_spectrum is not None):
            ...
            if (start == stop):
                ...
                self._centeral_wavelength_in_microns = float_round(
                    core.get_center_wavelen(self._scene_spectrum_in_microns[0], self._scene_spectrum_in_microns[1]))
        ...
        if (self._LENS_DEFINED and self._SENSOR_DEFINED):
            ...
            if self._sensor.QE is not None:
                ...
                self._wave_diffraction = 1e-3 * max(2 * self._diffraction_scalar * self._lambdas) * (
                            self._lens.focal_length / self._lens.diameter)
                # https://www.edmundoptics.com/knowledge-center/application-notes/imaging/limitations-on-resolution-and-contrast-the-airy-disk/
                print("----> Spot size because of the diffraction is {}[micro m]".format(
                    float_round(self._wave_diffraction)))
    ...
    def adjust_exposure_time(self, images):
        ...
        if (t < self._exposure_time):
            ...
            print("No adjustment was done, the exposure time still {} micro sec.".format(
    ...
    def get_GAMMA_lambda(self):
        ...
        * eta - camera system efficiency due to optics losses and sensor reflection (it is not a part of QE).
        The units of GAMMA_lambda are [electrons * m^2 * Sr /joule ].
        """
        GAMMA_lambda = 1e-12 * np.pi * self._Imager_EFFICIANCY * (
                    (self._lens.diameter / (2 * self._lens._FOCAL_LENGTH)) ** 2) * self._Imager_QE * (
                                   1e-9 * self._lambdas / (h * c)) * (self._sensor.pixel_size ** 2)
    ...
    def update_minimum_lens_diameter(self):
        ...
        if (np.isscalar(self._lambdas)):
            ...
            self._pixel_size_diffration = 1e-3 * (2 * self._diffraction_scalar * self._lambdas) * (
                        self._lens.focal_length / self._lens_diameter_diffration)  # in mirco m
        else:
            self._lens_diameter_diffration = 1e-6 * max(self._diffraction_scalar * self._lambdas) / Dtheta  # in mm
            self._pixel_size_diffration = 1e-3 * max(2 * self._diffraction_scalar * self._lambdas) * (
                        self._lens.focal_length / self._lens_diameter_diffration)  # in mirco m

        if (self._pixel_size_diffration > self._sensor.pixel_size):
            print("Diffreaction limit is met, The spot size is {} [micro m]".format(
                float_round(self._pixel_size_diffration)))
        ...
        print("----> Spot size because of the diffraction is changed to {}[micro m]".format(
            float_round(self._wave_diffraction)))
    ...
    def set_exposure_time(self, time):
        ...
        if (time is not None):
            ...
            if (self._SENSOR_DEFINED):  # means that the sensor should be defined
                ...
                if ((self._DARK_NOISE is not None) and (self._READ_NOISE is not None) and (
                        self._sensor.full_well is not None)):
                    ...
                    self._SNR = (self._sensor.full_well) / np.sqrt(
                        self._sensor.full_well + DARK_NOISE + (self._READ_NOISE ** 2) + (
                                    QUANTIZATION_NOISE ** 2))  # see https://www.photometrics.com/learn/imaging-topics/signal-to-noise-ratio
    ...
    def set_lens_diameter(self, diameter):
        ...
        print("The diameter of the lens will change in this step, Is that what do you want to do?")
        print("----> Diameter is changed to {}[mm]".format(float_round(self._lens.diameter)))
        print("----> Dynamic range is set to {} or {}[db]".format(float_round(self._DR),
                                                                  float_round(20 * np.log10(self._DR))))
    ...
    def update_solar_angle(self, val):
        ...
        """
        assert 90.0 < val <= 180.0, 'Solar zenith:{} is not in range (90, 180] (photon direction in degrees)'.format(
            val)
    ...
    def calculate_scene_solar_irradiance(self):
        ...
        LTOA = 6.8e-5 * 1e-9 * plank(1e-9 * self._lambdas)  # units fo W/(m^2 nm)
        ...
        if (self._LENS_DEFINED):  # means the lens should be defined
            ...
            if (self._lambdas is not None):
                ...
                if (np.isscalar(self._lambdas)):
                    ...
                    self._lens_diameter_diffration = 1e-6 * max(
                        self._diffraction_scalar * self._lambdas) / Dtheta  # in mm
                    self._pixel_size_diffration = 1e-3 * max(2 * self._diffraction_scalar * self._lambdas) * (
                                self._lens.focal_length / self._lens_diameter_diffration)  # in mirco m
                if (self._pixel_size_diffration > self._sensor.pixel_size):
                    print("Diffreaction limit is met, The spot size is {} [micro m]".format(
            ...
            if ((self._DARK_NOISE is not None) and (self._READ_NOISE is not None)):
                ...
                self._SNR = (self._sensor.full_well) / np.sqrt(
                    self._sensor.full_well + DARK_NOISE + (self._READ_NOISE ** 2) + (QUANTIZATION_NOISE ** 2))
                # self._SNR = (self._sensor.full_well)**0.5 old version
                ...
                print("To be outside the diffreaction limit, the lens diameter should be larger than {} mm".format(
                    float_round(self._lens_diameter_diffration)))
                print("----> Exposure bound is set to {}[micro sec]".format(float_round(self._max_exposure_time)))
                print("----> Dynamic range (at full well) bound is set to {} or {}[db]".format(float_round(self._DR),
                                                                                               float_round(
                                                                                                   20 * np.log10(
                                                                                                       self._DR))))
                print("----> Noise floor bound is set to {}[electrons]".format(float_round(self._NOISE_FLOOR)))
                print("----> SNR (at full well) is {} or {}[db]".format(float_round(self._SNR),
                                                                        float_round(self._sensor.get_SNR_IN_DB())))
    ...
    def set_scene_spectrum_in_microns(self, scene_spectrum):
        ...
        if (self._LENS_DEFINED):  # means we have here defined lens
            if (np.isscalar(self._lambdas)):
                ...
                self._wave_diffraction = 1e-3 * max(2.44 * self._lambdas) * (
                            self._lens.focal_length / self._lens.diameter)
    ...
    def ImportConfig(cls, file_name='Imager_config.json'):
        ...
        TRANSMISSION = pd.DataFrame(
            data={"<wavelength [nm]>": data['SPECTRUM'], '<Efficiency>': data['LENS_TRANSMISSION']}, index=None)
        if (data['SENSOR_TYPE'] is 'SIMPLE'):
            lens = None
        ...
        if (data['SENSOR_TYPE'] is not 'SIMPLE'):
            ...
            obj._ETA = data[
                'SYSTEM_EFFICIENCY']  # It is the camera system efficiency, I don't know yet how to set its value,
            ...
            obj._centeral_wavelength = float_round(
                core.get_center_wavelen(obj._scene_spectrum[0], obj._scene_spectrum[1]))
            obj._centeral_wavelength_in_microns = float_round(
                core.get_center_wavelen(obj._scene_spectrum_in_microns[0], obj._scene_spectrum_in_microns[1]))
    ...
    def ExportConfig(self, file_name='Imager_config.json', force=False):
        ...
        _DICT_[
            'DYNAMIC_RANGE'] = self._sensor.DynamicRange if self._sensor is not None else None  # unitless, it is not in DB
        ...
        new_dict['temp'] = self._dark_noise_table[
            'temp'].values.tolist() if self._dark_noise_table is not None else None
        new_dict['noise [electrons/sec]'] = self._dark_noise_table[
            'noise [electrons/sec]'].values.tolist() if self._dark_noise_table is not None else None
        ...
        _DICT_['MAX_EXPOSURE_TIME'] = int(
            self._max_exposure_time) if self._max_exposure_time is not None else None  # in [micro sec]
        _DICT_['START_SPECTRUM'] = self._scene_spectrum[0] if (
                    self._scene_spectrum is not None and self._sensor is not None) else None  # in nm
        _DICT_['END_SPECTRUM'] = self._scene_spectrum[1] if (
                    self._scene_spectrum is not None and self._sensor is not None) else None  # in nm
        _DICT_['SPECTRUM'] = self._lambdas.tolist() if (
                    self._lambdas is not None and self._sensor is not None) else None  # in nm
        ...
        if (force is False):  # force the export without anyy chacking of the content.
            ...
            if (self._radiance is not None):
                ...
                G1 = (1 / (h * c)) * self._exposure_time * (((np.pi) / 4) / (self._lens._FOCAL_LENGTH ** 2)) * (
                            self._sensor.pixel_size ** 2)
                G2 = np.trapz((self._radiance * self._Imager_EFFICIANCY * self._Imager_QE) * self._lambdas,
                              x=self._lambdas)
                G = 1e-21 * G1 * G2
            ...
            if (self._DR is not None):
                print("----> Dynamic range is {} or {}[db]".format(float_round(self._DR),
                                                                   float_round(20 * np.log10(self._DR))))
...
def test_gecko_like_imager():
    ...
    # Define sensor:
    sensor = shdom.SensorFPA(PIXEL_SIZE=GECKO['PIXEL_SIZE'], FULLWELL=GECKO['FULLWELL'], CHeight=GECKO['CHeight'],
                             CWidth=GECKO['CWidth'],
                             SENSOR_ID=GECKO['SENSOR_ID'], READOUT_NOISE=GECKO['READOUT_NOISE'],
                             DARK_CURRENT_NOISE=GECKO['DARK_CURRENT_NOISE'], TEMP=GECKO['TEMP'],
                             BitDepth=GECKO['BitDepth'])
