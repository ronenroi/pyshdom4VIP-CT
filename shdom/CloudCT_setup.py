...
class SpaceMultiView_Measurements(object):
    ...
    def __init__(self, setup_of_views_list=None):
        ...
        self._imagers_unique_wavelengths_list = [float_round(w.imager.centeral_wavelength_in_microns) for w in
                                                 self._setup_of_views_list]  # Actualy the list may be not uniqe if for instance 2 imagers have the same centeral wavelength.
    ...
    def simulate_measurements(self, rte_solvers=None, n_jobs=1, IF_APPLY_NOISE=False, IF_SCALE_IDEALLY=False,
        ...
        for wavelength in Solvers_dict.keys():
            ...
            if (wavelength in self._imagers_unique_wavelengths_list):
                ...
                warnings.warn(
                    "It is very strange that the solver does not have a wavelength that exists in the images list.\nMaybe you deactivate an imager? if not, you have a bug in the setup definition.")
            ...
            if (IF_REDUCE_EXPOSURE):
                projections.imager.adjust_exposure_time(images_list_per_imager,
                                                        C=0.9)  # C = 0.9 - the imager will reach 90 procent of the full well.

            self._Images_per_imager[
                imager_index], radiance_to_graylevel_scale = projections.imager.convert_radiance_to_graylevel(
                images_list_per_imager, IF_APPLY_NOISE=IF_APPLY_NOISE, IF_SCALE_IDEALLY=IF_SCALE_IDEALLY)
        ...
        for imager_index in self._Images_per_imager.keys():
            ...
            if (self._Images_per_imager[imager_index] is not None):
                ...
                if (nrows_ncols == [1, 1]):
                    ...
                    image = images[0].copy() * \
                            self._radiance_to_graylevel_scales[
                                imager_index]  # to set the images in the grayscale level.
                    ...
                    for projection_index, (ax, name) in enumerate(zip(grid, projections.projection_names)):
                        ...
                        image *= self._radiance_to_graylevel_scales[
                            imager_index]  # to set the images in the grayscale level..
    ...
    def convert2regular_measurements(self):
        ...
        sensor = shdom.RadianceSensor()  # Currently, this method sopports only sensor of type shdom.RadianceSensor().
        assert len(
            self._imagers_unique_wavelengths_list) == 1, "If the case is polychromatic, it must be a mistake, you should not try to convert the measurments. It works only for monochromatic!"
...
class SpaceMultiView(shdom.MultiViewProjection):
    ...
    def update_views(self, names=None):
        ...
        for pos, lookat, up, samples_per_pixel, name, Flag in zip(self._sat_positions, \
                                                                  self._lookat_list, up_list, self._rays_per_pixel,
                                                                  names, self._Imager_config):
...
def satpositions(ANG_APERTURE_FROM_SURFACE=None, R_SAT=500):
    if ANG_APERTURE_FROM_SURFACE == None:
        ANG_APERTURE_FROM_SURFACE = 50 * (np.pi / 180)  # [radians]
    SAT_DIST = 100e3  # [m]
    R_EARTH = 6371000  # 6378.1e3  # [m]
    R_SAT *= 1e3  # from [km] to [m]

    dtheta = SAT_DIST / (R_EARTH + R_SAT)  # angle resulting from 100km arc

    # calc maximal possible aperture (theta angle) from center of the Earth using the aperture from the surface:
    # This calculation results from a trigonometric solution
    max_ang_ap = np.arcsin(
        -(R_EARTH / (R_EARTH + R_SAT)) * np.sin(ANG_APERTURE_FROM_SURFACE)) + ANG_APERTURE_FROM_SURFACE

    # calc how many satellite rings can be inserted while conserving the 100km limit
    arc_len = max_ang_ap * (R_EARTH + R_SAT)
    rings_num = int(np.floor(arc_len / SAT_DIST))
    rings_vec = [*range(1, rings_num + 1)]

    theta_vec = np.array([float(i) * dtheta for i in rings_vec])  # theta angle for each satellite ring
    xy_r_vec = (R_EARTH + R_SAT) * np.sin(theta_vec)  # the satellite rings' radii
    z_vec = (R_EARTH + R_SAT) * np.cos(theta_vec)  # z coordinate for each satellite ring

    sats_per_ring = np.floor(2 * np.pi * xy_r_vec / SAT_DIST)  # number of satellites on each ring
    tot_num_of_sats = np.sum(sats_per_ring) + 1  # plus one because of the satellite in x,y=0,0
    dphi_per_ring = 2 * np.pi / sats_per_ring  # for each ring: the angle between each pair of adjacent satellites

    # calc the satellite locations by iterating on each ring
    sat_locations = [[0, 0, R_EARTH + R_SAT]]  # center satellite
    for i, xy_r in enumerate(xy_r_vec):
        phi_vec = dphi_per_ring[i] * np.array([*range(int(sats_per_ring[i]))])
        for phi in phi_vec:
            sat_locations.append([xy_r * np.cos(phi), xy_r * np.sin(phi), z_vec[i]])

    final_sat_locations = np.array(sat_locations)
    final_sat_locations[:, 2] = final_sat_locations[:, 2] - R_EARTH  # subtracting R_EARTH from Z to get dist from earth
    return (final_sat_locations / 1e3) # back to [km]
    # ------ Plot Generation ------ #
    # fig = plt.figure()
    # ax = fig.add_subplot(projection='3d')
    # ax.scatter(final_sat_locations[:, 0], final_sat_locations[:, 1], final_sat_locations[:, 2])
...
def StringOfPearls(SATS_NUMBER=10, orbit_altitude=500, axis='y'):
    """
    Set orbit parmeters:
         input:
    ...
    if axis=='y':
        ...
        Z_config = r_orbit * np.cos(theta_config) - r_earth
        Y_config = r_orbit * np.sin(theta_config)
        X_config = np.zeros_like(Y_config)

        sat_positions = np.vstack([X_config, Y_config, Z_config])  # path.shape = (3,#sats) in km.

        # find near nadir view:
        # since in this setup x=0:
        near_nadir_view_index = np.argmin(np.abs(Y_config))
    return sat_positions.T, near_nadir_view_index


def Create(SATS_NUMBER=10, ORBIT_ALTITUDE=500, SAT_LOOKATS=None, Imager_config=None, imager=None, samples_per_pixel=1,
           rigid_sampling=False, VISSETUP=False):
    """
    ...
    MV = SpaceMultiView(imager, Imager_config, samples_per_pixel,
                        rigid_sampling)  # it is the geomerty for spesific imager
    MV.set_satellites_position_and_lookat(sat_positions,
                                          sat_lookats)  # here we set only the positions and where the satellites are lookin at.
    MV.update_views()
...
def CreateVaryingImagers(SATS_NUMBER=10, ORBIT_ALTITUDE=500, SAT_LOOKATS=None, Imager_config=None, imager=None, samples_per_pixel=1,
           rigid_sampling=False, VISSETUP=False, DX=0,DY=0,DZ=0,N=1):
    """
    Create the Multiview setup on orbit direct them with lookat vector and set the Imagers at thier locations + orientations.
    The output here will be a list of Imagers. Each Imager will be updated here with respect to the defined geomtric considerations.
    ...
    sat_positions, near_nadir_view_index = StringOfPearls(SATS_NUMBER=SATS_NUMBER, orbit_altitude=ORBIT_ALTITUDE)
    sat_positions = np.dstack([sat_positions]*N)
    ...
    sat_positions[:,1,:] += dy
    sat_positions[:,2,:] += dz
    sat_positions = np.swapaxes(sat_positions,1,2).reshape((-1,3))
    assert imager is not None, "You must provide the Imager object!"
    ...
    if SAT_LOOKATS is None:
        ...
        sat_lookats = np.repeat(SAT_LOOKATS,N,0)
...
def Create_XY(SATS_NUMBER=10, ORBIT_ALTITUDE=500, SAT_LOOKATS=None, Imager_config=None, imager=None, samples_per_pixel=1,
           rigid_sampling=False, VISSETUP=False):
    """
    Create the Multiview setup on orbit direct them with lookat vector and set the Imagers at thier locations + orientations.
    The output here will be a list of Imagers. Each Imager will be updated here with respect to the defined geomtric considerations.
    ...
    """
    sat_positions_y, near_nadir_view_index = StringOfPearls(SATS_NUMBER=int(SATS_NUMBER/2), orbit_altitude=ORBIT_ALTITUDE, axis='y')
    sat_positions_x, _ = StringOfPearls(SATS_NUMBER=SATS_NUMBER-int(SATS_NUMBER/2), orbit_altitude=ORBIT_ALTITUDE,
                                                              axis='x')
    sat_positions = np.concatenate((sat_positions_y,sat_positions_x),0)
    assert imager is not None, "You must provide the Imager object!"
    ...
    return MV, near_nadir_view_index
def Create_DOM(ANG_APERTURE_FROM_SURFACE=None, ORBIT_ALTITUDE=500, SAT_LOOKATS=None, Imager_config=None, imager=None, samples_per_pixel=1,
           rigid_sampling=False, VISSETUP=False):

    sat_positions  = satpositions(ANG_APERTURE_FROM_SURFACE,R_SAT=ORBIT_ALTITUDE)
    near_nadir_view_index = 0
    SATS_NUMBER = sat_positions.shape[0]
    assert imager is not None, "You must provide the Imager object!"
    ...
    return MV, near_nadir_view_index
def old_Create(SATS_NUMBER=10, ORBIT_ALTITUDE=500, CAM_FOV=0.1, CAM_RES=(64, 64), SAT_LOOKATS=None,
               SATS_WAVELENGTHS_LIST=None, SOLAR_FLUX_LIST=None, VISSETUP=False):
    """
...
def Prepare_Medium(CloudFieldFile=None, AirFieldFile=None, MieTablesPath=None,
    ...
    if (AirFieldFile is None):
        print(
            "You did not provied the air field for the simulation. The atmospher will not include Molecular Rayleigh scattering.")
