...
class RadianceSensor(Sensor):
    ...
    def render(self, rte_solver, projection, n_jobs=1, verbose=0):
        ...
        radiance = np.concatenate(radiance,0).reshape((-1,)) #radiance[0]
...
class PerspectiveProjection(HomographyProjection):
    ...
    def __init__(self, fov, nx, ny, x, y, z, samples=1, rigid_sampling=False):
        ...
        x_c, y_c, z_c = np.meshgrid(np.linspace(-R[0], R[0] - self._dx, nx), np.linspace(-R[1], R[1] - self._dy, ny),
                                    1.0)
        ...
        if (self._samples_per_pixel > 1):
            if (rigid_sampling):
                ...
                self._samples_per_pixel = int(
                    self._Q ** 2)  # update with reasonable samples per pixel to enable square uniform sampling
                Z = np.zeros_like(x_c)
                    ...
                    for iy in range(ny):
                        for ky in range(self._Q):
                            for kx in range(self._Q):
                                ...
                                ind_n = np.ravel_multi_index([ix, iy, ky, kx],
                                                             (nx, ny, self._Q, self._Q))  # index in the new grid
        ...
        G = np.hstack([self._rotation_matrix.T, -self._rotation_matrix.T @ self._position.reshape(3, 1)])
        self.projection_matrix = self._k @ G
